...
class plgAcymailingTemplate extends JPlugin{
	...
	public function acymailing_replaceusertags(&$email, &$user, $send = true){
		...
		if((!acymailing_level(1) || acymailing_level(4)) && !empty($email->type) && in_array($email->type, array('news', 'followup'))){
			$pict = '<div style="text-align:center;margin:10px auto;display:block;"><a target="_blank" href="'.ACYMAILING_ACYWEBSITE.'?utm_source=acymailing&utm_medium=e-mail&utm_content=img&utm_campaign=powered-by"><img alt="Powered by AcyMailing" src="media/com_acymailing/images/poweredby.png" /></a></div>';
