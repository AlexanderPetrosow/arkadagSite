...
class plgAcymailingTagsubscription extends JPlugin
{
    ...
    function acymailing_replaceusertags(&$email, &$user, $send = true)
    {
        ...
        if (!empty($user) && !empty($this->unsubscribeLink[$email->mailid]) && empty($this->addedListUnsubscribe[$email->mailid][$user->subid]) && $this->params->get('listunsubscribe', 0) && method_exists($email, 'addCustomHeader')) {
