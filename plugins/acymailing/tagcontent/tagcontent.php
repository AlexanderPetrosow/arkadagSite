...
class plgAcymailingTagcontent extends JPlugin
{
    public function __construct(&$subject, $config)
    {
        ...
        if (version_compare(ACYMAILING_CMSV, '3.4.0', '>=')) {
            if (JComponentHelper::isInstalled('com_multicats') && JComponentHelper::isEnabled('com_multicats')) $this->multicatsOn = true;
        } else {
            try {
                if (file_exists(JPATH_ADMINISTRATOR.DS.'com_multicats') && JComponentHelper::isEnabled('com_multicats')) $this->multicatsOn = true;
            } catch (Exception $exc) {
                $this->multicatsOn = false;
            }
        }
		...
		<div class="onelineblockoptions">
			<table width="100%" class="acymailing_table">
				...
				<tr>
					...
					<td>
                        <?php
                        $values = ['id' => 'ACY_ID', 'ordering' => 'ACY_ORDERING', 'created' => 'CREATED_DATE', 'modified' => 'MODIFIED_DATE', 'title' => 'FIELD_TITLE', 'hits' => 'ACY_HITS'];
                    ...
                    foreach ($oneField['values'] as &$oneFieldVal) {
                        switch ($oneField['field']->type) {
                            ...
                            case 'calendar':
                                ...
                                if (!empty($tag->{$oneField['field']->name.'format'})) $format = acymailing_translation($tag->{$oneField['field']->name.'format'});
    ...
    public function acymailing_generateautonews(&$email)
    {
        ...
        foreach ($tags as $oneTag => $parameter) {
            ...
            if ($this->multicatsOn && $this->newMulticats && (!empty($selectedArea) || !empty($parameter->excludedcats))) $query .= ' JOIN `#__multicats_content_catid` as mcc ON a.id = mcc.item_id ';
