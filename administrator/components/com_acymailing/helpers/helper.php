...
define('ACYMAILING_ACYWEBSITE', 'https://www.acyba.com/');
define('ACYMAILING_UPDATEMEURL', ACYMAILING_ACYWEBSITE.'index.php?option=com_updateme&ctrl=');
define('ACYMAILING_UPDATEURL', ACYMAILING_UPDATEMEURL.'update&task=');
define('ACYMAILING_SPAMURL', ACYMAILING_UPDATEMEURL.'spamsystem&task=');
define('ACYMAILING_HELPURL', ACYMAILING_UPDATEMEURL.'doc&component='.ACYMAILING_NAME.'&version=5&page=');
define('ACYMAILING_REDIRECT', ACYMAILING_UPDATEMEURL.'redirect&page=');
...
function acymailing_getDate($time = 0, $format = '%d %B %Y %H:%M')
{
    ...
    if (ACYMAILING_J16) {
        $format = str_replace(['%A', '%d', '%B', '%m', '%Y', '%y', '%H', '%M', '%S', '%a', '%I', '%p', '%w'], ['l', 'd', 'F', 'm', 'Y', 'y', 'H', 'i', 's', 'D', 'h', 'a', 'w'], $format);
        try {
...
function acymailing_absoluteURL($text)
{
    ...
    $text = str_replace(['href="../undefined/', 'href="../../undefined/', 'href="../../../undefined//', 'href="undefined/', ACYMAILING_LIVE.'http://', ACYMAILING_LIVE.'https://'], ['href="'.$mainurl, 'href="'.$mainurl, 'href="'.$mainurl, 'href="'.ACYMAILING_LIVE, 'http://', 'https://'], $text);
...
function acymailing_footer()
{
    ...
    $text .= '<div class="acymailing_footer" align="center" style="text-align:center"><a href="'.ACYMAILING_ACYWEBSITE.'?utm_source=acymailing-'.$level.'&utm_medium=front-end&utm_content=txt&utm_campaign=powered-by" target="_blank" title="'.ACYMAILING_NAME.' : '.str_replace('TM ', ' ', strip_tags($description)).'">'.ACYMAILING_NAME;
...
function acymailing_getFiles($path, $filter = '.', $recurse = false, $full = false, $exclude = ['.svn', 'CVS', '.DS_Store', '__MACOSX'], $excludefilter = ['^\..*', '.*~'], $naturalSort = false)
{
    $path = acymailing_cleanPath($path);
